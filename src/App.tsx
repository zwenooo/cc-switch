...
// 构建期开关：VITE_HIDE_CLAUDE=1/true/yes 时隐藏 Claude（含 Claude Desktop），默认切到 Codex
const HIDE_CLAUDE = (() => {
...
const isHiddenApp = (app: AppId): boolean =>
  HIDE_CLAUDE && (app === "claude" || app === "claude-desktop");

const getInitialApp = (): AppId => {
  const saved = localStorage.getItem(STORAGE_KEY) as AppId | null;
  if (saved && VALID_APPS.includes(saved) && !isHiddenApp(saved)) {
    return saved;
  }
  return HIDE_CLAUDE ? "codex" : "claude";
};
...
function App() {
  ...
  const visibleApps: VisibleApps = (() => {
    const base = settingsData?.visibleApps ?? {
      claude: true,
      "claude-desktop": true,
      codex: true,
      gemini: true,
    ...
    // 构建期隐藏 Claude：强制隐藏，优先级高于用户设置
    return HIDE_CLAUDE
      ? { ...base, claude: false, "claude-desktop": false }
      : base;
  })();
